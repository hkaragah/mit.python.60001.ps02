...
def match_with_gaps(my_word, other_word):
    ...
    :param my_word: string, the word guessed by user
    :param other_word: string, other words in the list to be compared against 'my_word'
    :return: return True if the guessed letters of my_word match the corresponding letters of other_word ...
    It should return False if the two words are not of the same length or ...
    if a guessed letter in my_word does not match the corresponding character in other_word.
    '''
    ...
    if len(my_word) == len(other_word):
        for index in range(len(my_word)):
            if my_word[index] != other_word[index] and my_word[index] != '_':
                return False
    else:
        return False
...
def show_possible_matches(my_word, wrong_guesses):
    ...
    """
    :param my_word: string, the word guessed by user
    :param wrong_guesses: list, list of letter guessed wrong by user
    :return: none
    This function should print out all words in 'wordlist' ...
    that match 'my_word' . It should print “No matches found” if there are no matches.
    """
...
def hangman(secret_word):
    ...
    * After each guess, you should display to the user the
    ...
    while guesses_remaining > 0:
        ...
        # checking the guessed letter and updating the remaining warnings and guesses
        if not (str.isalpha(answer)):
            if answer == '*':   # asking for hint with no penalty
                show_possible_matches(my_word, wrong_guesses)  # printing the possible matches
        ...
        elif already_guessed(answer, letters_guessed):
            ...
            my_word = get_guessed_word(secret_word, letters_guessed)
            if is_letter_guessed(answer, secret_word):
